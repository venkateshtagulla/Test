...
def parse_lambda_response(result: dict) -> JSONResponse:
    """
    Convert Lambda-style response (with string 'body') into FastAPI JSONResponse.
    """
    # Lambda handlers return: {"statusCode": int, "body": str}
    body_dict = json.loads(result.get("body", "{}"))
    return JSONResponse(status_code=result.get("statusCode", 200), content=body_dict)
...
async def login_inspector_route(request: Request):
    ...
    event = await lambda_to_fastapi_response(request)
    result= inspector_login(event, None)
    return parse_lambda_response(result)
...
async def login_admin_route(request: Request):
    ...
    event = await lambda_to_fastapi_response(request)
    result= admin_login_handler(event, None)
    return parse_lambda_response(result)
...
async def get_vessel_assignments_route(vessel_id: str, request: Request):
...
async def admin_delete_crew_route(crew_id: str, request: Request, authorization: Optional[str] = Header(None)):
