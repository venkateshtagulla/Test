...
from models.request.inspection_form import (
    CreateInspectionFormRequest,
    GetInspectionFormRequest,
    ListInspectionFormsRequest,
    ListSubmittedFormsRequest,
    SubmitInspectionFormRequest,
    UpdateInspectionFormRequest,
    DeleteInspectionFormRequest,
)
...
def create_inspection_form_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=unused-argument
    ...
    try:
        ...
        payload = CreateInspectionFormRequest(**body)
        data = service.create_form(admin_id, payload)
...
def get_inspection_form_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=unused-argument
    ...
    try:
        ...
        payload = GetInspectionFormRequest(**params)
        data = service.get_form_by_id(payload, inspection_id=inspection_id)
...
def update_inspection_form_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=unused-argument
    ...
    try:
        ...
        payload = UpdateInspectionFormRequest(form_id=form_id, **body)
...
def delete_inspection_form_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=unused-argument
    ...
    try:
        ...
        payload = DeleteInspectionFormRequest(form_id=form_id)
        data = service.deactivate_form(admin_id, payload.form_id)
...
def list_inspection_forms_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=unused-argument
    ...
    try:
        admin_id = _get_admin_id_from_event(event)
        query_params = event.get("queryStringParameters") or {}
        payload = ListInspectionFormsRequest(
...
def list_submitted_forms_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=unused-argument
    ...
    try:
        admin_id = _get_admin_id_from_event(event)
        query_params = event.get("queryStringParameters") or {}
        payload = ListSubmittedFormsRequest(
    ...
    try:
        ...
        if "multipart/form-data" in content_type.lower():
            ...
            for field_name, (file_bytes, file_name) in files.items():
                ...
                # Upload to S3 and store URL as answer
                raw_url = upload_file_to_s3(
                    file_content=file_bytes,
                    file_name=file_name,
                ...
                answers_by_order[order] = s3_url

            if not answers_by_order:
                raise ApiError("At least one answer is required", 400, "missing_answers")

            # Convert to required format for service layer
            merged_answers = [
                {"order": order, "value": value}
                for order, value in sorted(answers_by_order.items())
            ]
        ...
        payload = SubmitInspectionFormRequest(form_id=form_id, **body)
