...
def admin_create_inspector_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=unused-argument
    ...
    try:
        ...
        if "multipart/form-data" in content_type.lower():
            ...
            data = service.create_inspector(payload_data, files=files if files else None)
        else:
            # Standard JSON request
            body = _parse_body(event)
            # Manual Validation for JSON
            missing_fields = [
                field
                for field in ("first_name", "last_name", "email", "password")
            ...
            data = service.create_inspector(body)
...
def admin_create_inspector_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=unused-argument
    ...
    try:
        ...
        if "multipart/form-data" in content_type.lower():
            ...
            data = service.create_inspector(admin_id, payload.model_dump(), files=files if files else None)
        else:
            # Standard JSON request
            body = _parse_body(event)
...
def admin_get_inspector_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=unused-argument
    ...
    try:
        ...
        # Ensure inspector_id exists
        if not params.get("inspector_id"):
             raise ApiError("inspector_id is required", 400, "missing_param")
             
        data = service.get_inspector(params)
...
def admin_reset_inspector_password_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=unused-argument
    ...
    try:
        ...
        if not payload_data.get("password"):
            raise ApiError("password is required in body", 400, "missing_password")
        
        data = service.reset_password(payload_data)
