...
def presign_upload_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:  # pylint: disable=unused-argument
    ...
    try:
        ...
        if not file_name or not content_type:
            raise ApiError("Missing file_name or content_type", 400, "missing_fields")
            
        prefix = folder.rstrip("/") if folder else "uploads"
        key = f"{prefix}/{admin_id}/{uuid4()}_{file_name}"
        presign = generate_presigned_put_url(key=key, content_type=content_type)
